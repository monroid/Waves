...
import scorex.transaction.{FeeCalculator, History, Transaction}
import scorex.utils.Time

import scala.concurrent.duration._

class UtxPoolSpecification extends FreeSpec
  with Matchers with MockFactory with PropertyChecks with TransactionGen with NoShrink {
  ...
  private def mkState(senderAccount: Address, senderBalance: Long) = {
    ...
    val (history, state, bcu) = StorageFactory(settings, ???, ???)
