...
class PostgresWriter(ds: DataSource) extends SnapshotStateReader with StateWriter {
  ...
  override def nonZeroLeaseBalances = readOnly { implicit s =>
    sql"""with last_balance as (select address, max(height) height from lease_balances group by address)
      ...
      .map(rs => Address.fromString(rs.get[String](1)).right.get -> LeaseBalance(rs.get[Long](2), rs.get[Long](3)))
      .list()
      .apply()
      .toMap
  }
  ...
  override def leaseBalance(a: Address) = readOnly { implicit s =>
