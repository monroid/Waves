...
import com.wavesplatform.state2.{AssetDescription, AssetInfo, BalanceSnapshot, ByteStr, LeaseBalance, VolumeAndFee}
...
class SQLiteWriter(ds: DataSource, fs: FunctionalitySettings) extends Caches {
  ...
  override protected def loadWavesBalance(address: Address): Long = roOpt(q.wavesBalance, address.bytes.arr).getOrElse(0)

  override protected def loadLeaseBalance(address: Address): LeaseBalance = roOpt(q.leaseBalance, address.bytes.arr).getOrElse(LeaseBalance.empty)

  override protected def loadAssetBalance(address: Address): Map[AssetId, Long] = roMap(q.assetBalance, address.bytes.arr)
  ...
  override def nonZeroLeaseBalances = readOnly { implicit s =>
    sql"""with last_balance as (select address, max(height) height from lease_balances group by address)
      ...
      .map(rs => rs.get[Address](1) -> LeaseBalance(rs.get[Long](2), rs.get[Long](3)))
      .list()
      .apply()
      .toMap
  }
  ...
  override def aliasesOfAddress(a: Address) = roSeq(q.aliasesOfAddress, a.bytes.arr)
  ...
  override def leaseOverflows = readOnly { implicit s =>
    sql"""with
      ...
      .map(rs => rs.get[Address](1) -> rs.get[Long](2))
      .list()
      .apply()
      .toMap
  }
  ...
  override def leasesOf(address: Address) = readOnly { implicit s =>
    sql"""with latest_lease_status as (select lease_id, min(active) active from lease_status group by lease_id)
      ...
      .bind(address.bytes.arr)
      .map(rs => rs.get[ByteStr](1) -> LeaseDetails(
        rs.get[PublicKeyAccount](2),
        rs.get(3)(addressOrAlias),
        rs.get[Int](4),
        rs.get[Long](5),
        rs.get[Boolean](6)))
      .list()
  ...
  override def doAppend(block: Block,
