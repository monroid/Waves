...
//    GlobalSettings.loggingSQLAndTime = LoggingSQLAndTimeSettings(enabled = true)
      ...
      case Success(filename) =>
        ...
        createInputStream(filename) match {
          case Success(inputStream) =>
            val (history, _, blockchainUpdater) = StorageFactory(settings, createDataSource(config.getConfig("waves.database")), NTP)
